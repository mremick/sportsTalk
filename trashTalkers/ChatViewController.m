...
- (BOOL)textFieldShouldReturn:(UITextField *)textField
{
    ...
    if ([self.chatTextField.text length] > 0) {
        ...
        [self.chatData addObject:newPost];
...
{
    Post *currentPost = [self.chatData objectAtIndex:self.chatData.count-indexPath.row-1];
...
- (void)loadLocalChat
{
    ...
    [postsRelation.query orderByDescending:@"updatedAt"];
                ...
                [userQuery findObjectsInBackgroundWithBlock:^(NSArray *objects, NSError *error) {
                    if (!error) {
                        if ([objects count]) {
                            ...
                            NSLog(@"added objects in chatdata");
